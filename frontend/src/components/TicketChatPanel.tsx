...
export const TicketChatPanel: React.FC<TicketChatPanelProps> = ({
  ...
  const canCreateChat =
    ticket.assignees?.some((a: any) => a.id === user?.id) || user?.is_superuser;
              ...
              actions={
                canManage && p.user.id !== user?.id
                  ? [
                      <Popconfirm
                        title="Remove user?"
                        onConfirm={() => handleRemoveParticipant(p.user.id)}
                      >
                        <Button
                          type="text"
                          size="small"
          ...
          <Space direction="vertical" style={{ width: "100%" }}>
            ...
            >
              {availableUsers
                .filter(
                  (u) =>
                    !currentParticipants.some((p: any) => p.user.id === u.id)
                )
                .map((u) => (
            ...
            <Space>
              ...
              <Button size="small" onClick={() => setAddingParticipant(false)}>
                Cancel
              </Button>
            </Space>
          </Space>
